...
exports.updateEventAttendee = catchAsync(async (req, res, next) => {
  ...
  if (
    ...
    new Date(currentEventAttendee.modifiedDateTime)
  ) {
    return next(
      new AppError(
...
exports.updateThisAndFollowingEventAttendees = catchAsync(
  async (req, res, next) => {
    ...
    if (
      new Date(currentEventAttendee.startDateTime).getDay() !=
      new Date(eventAttendeeToCreate.startDateTime).getDay()
    ) {
      eventExceptions = await EventException.deleteMany({
        eventAttendeeID: { $eq: req.params.id },
      })
    } else {
      eventExceptions = await EventException.updateMany(
        ...
        { eventAttendeeID: newEventAttendee._id },
...
exports.updateAllEventAttendees = catchAsync(async (req, res, next) => {
  ...
  if (
    new Date(currentEventAttendee.startDateTime).getDay() !=
    new Date(eventAttendee.startDateTime).getDay()
  ) {
...
exports.deleteThisAndFollowingEventAttendees = catchAsync(
  async (req, res, next) => {
    let untilDate = new Date(req.body.startDateTime)
    untilDate.setDate(untilDate.getDate() - 1)
